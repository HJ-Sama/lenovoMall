...
public class OrderDao {
	...
	public Order findOrderById(String id) throws SQLException {
		String sql = "select * from orders,customer where orders.user_id=customer.id and orders.id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new ResultSetHandler<Order>() {
			public Order handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					User user = new User();
					user.setId(rs.getInt("customer.id"));
					user.setEmail(rs.getString("customer.email"));
					user.setGender(rs.getString("customer.gender"));
//					user.setActiveCode(rs.getString("customer.activecode"));
//					user.setIntroduce(rs.getString("customer.introduce"));
					user.setPassword(rs.getString("customer.password"));
					user.setRegistTime(rs.getDate("customer.registerTime"));
					user.setRole(rs.getString("customer.role"));
					user.setState(rs.getInt("customer.state"));
					user.setTelephone(rs.getString("customer.telephone"));
					user.setUsername(rs.getString("customer.username"));
					order.setUser(user);
				}
				return order;
			}
	...
	public List<Order> findAllOrder() throws SQLException {
		//1.创建sql
		String sql = "select orders.*,customer.* from orders,customer where customer.id=orders.user_id order by orders.user_id";
		//2.创建QueryRunner对象
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
...
//					user.setIntroduce(rs.getString("customer.introduce"));
	...
	public List<Order> findOrderByManyCondition(String id, String receiverName)
		...
		//2.定义查询sql
		String sql = "select orders.*,customer.* from orders,customer where customer.id=orders.user_id ";
		//3.根据参数拼接sql语句
