...
import edu.ouc.lenovo.dao.ProductDao;
import edu.ouc.lenovo.dao.ProductImgDao;
import edu.ouc.lenovo.domain.PageBean;
import edu.ouc.lenovo.domain.Product;
import edu.ouc.lenovo.domain.ProductImg;
import edu.ouc.lenovo.exception.AddProductException;
import edu.ouc.lenovo.exception.FindProductByIdException;
import edu.ouc.lenovo.exception.ListProductException;
public class ProductService {
	private ProductDao dao = new ProductDao();
	private ProductImgDao imgdao = new ProductImgDao();
	// 添加商品
	...
	// 添加商品图片
	public void addProductImg(ProductImg pi) throws AddProductException {
		try {
			imgdao.addProductImg(pi);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new AddProductException("添加商品失败");
		}
	...
	// 根据id查找商品图片
	public List<ProductImg> findProductImgById(String id) throws FindProductByIdException {
		try {
			return imgdao.findProductImgById(id);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new FindProductByIdException("根据ID查找商品图片失败");
		}
	}
	// 下载销售榜单
