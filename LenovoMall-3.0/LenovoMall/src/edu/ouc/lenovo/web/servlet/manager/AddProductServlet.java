...
import edu.ouc.lenovo.utils.FileUploadUtils;
import edu.ouc.lenovo.utils.IdUtils;
/**
 * 后台系统
 * 用于添加商品的servlet
 */
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
				...
				if (item.isFormField()) {
					...
					if(imgurl.contains(".")){
						...
						if(count == -1) {
							...
							map2.put("url", imgurl);
							map2.put("img_order",String.valueOf(count));
							HashMap<String, String> map3 = new HashMap<String, String>();
							map3.putAll(map2);
							imgurls.add(count,map3);
							count++;
							}	
					}
		...
		try {
			...
			if(count>=1) {
				...
				for(Map<String, String> url:imgurls) {
					if(url.get("url")==null)continue;
					ProductImg tmp=new ProductImg();
					BeanUtils.populate(tmp, url);
					productImgs.add(tmp);
				}
			}
